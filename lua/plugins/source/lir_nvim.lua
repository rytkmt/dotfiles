...
require'lir'.setup {
  ...
  mappings = {
    -- なんか開くときにエラーになって開けないバグ？があるからpcallでエラーをキャッチできないか試しで
    ['e']     = function() pcall(actions.edit) end,
    ['<CR>']  = function() pcall(actions.edit) end,
    ['o']     = function() pcall(actions.edit) end,
    ['l']     = function() pcall(actions.edit) end,
    ['s']     = function() pcall(actions.split) end,
    ['v']     = function() pcall(actions.vsplit) end,
    ['t']     = function() pcall(actions.tabedit) end,
    ['R']     = actions.reload,
    ['h']     = actions.up,
    ['<C-q>'] = function() pcall(actions.quit) end,
    ['<C-[>'] = function() pcall(actions.quit) end,
    ['q']     = function() pcall(actions.quit) end,
    ['a']     = input_newfile,
    -- ['ad']    = actions.mkdir,
    -- ['af']    = actions.newfile,
    ['r']     = actions.rename,
...
set_open_keymap("B", "vim.fn.expand('%:p:r')")
set_open_keymap("h", "vim.env.HOME")
set_open_keymap("v", "vim.env.DOT_FILES")
set_open_keymap("s", "vim.env.PRIVATE_DOTFILES_DIR..'/source_memo/'")
set_open_keymap("a", "vim.env.DOT_FILES..'/ai/'")
set_open_keymap("c", "vim.fn.FindGitProjectRoot()..'/.claude/'")
