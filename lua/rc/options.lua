...
local group = vim.api.nvim_create_augroup("ToggleCursorLine", { clear = true })

-- ウィンドウに入ったとき：cursorline を無効にする
vim.api.nvim_create_autocmd({ "WinEnter", "BufEnter", "FocusGained" }, {
  group = group,
  pattern = "*",
  callback = function()
    if vim.tbl_contains({ 'lir' }, vim.bo.filetype) then
      return
    end
