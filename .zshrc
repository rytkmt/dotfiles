...
if [[ $(command -v fzf) ]]; then
  ...
  function fzf-cdr () {
    local selected_dir="$(cdr -l | sed 's/^[0-9]* *//' | fzf --reverse)"
  ...
  function fzf-find-cd() {
    local selected_dir=$(find . -type d | fzf --reverse)
    if [ -n "$selected_dir" ]; then
      BUFFER="cd ${selected_dir}"
      zle accept-line
    fi
  ...
  function fzf-src () {
    ...
    if [ -n "$dir_array" ]; then
      for dir in "${dir_array[@]}"; do
        ...
        command+="find ${dir} -maxdepth 1 -mindepth 1 -type l -print;"
        sed_command+="s|^${dir}/||;"
      done
    fi
    local selected_dir=$(eval $command | sed "$sed_command" | fzf --reverse)
    if [ -n "$selected_dir" ]; then
      for dir in "${dir_array[@]}"; do
        if [ -e "${dir}/${selected_dir}" ]; then
          BUFFER="cd ${dir}/${selected_dir}"
          zle accept-line
          return
        fi
      done
...
function md2pdf() {
  ...
  if [[ -z "$target" ]]; then
    output="$(dirname "$input")/$(basename "${input%.md}.pdf")"
  elif [[ "$target" == */ || -d "$target" ]]; then
    mkdir -p "$target"
    output="${target%/}/$(basename "${input%.md}.pdf")"
  else
  ...
  local stderr
  stderr=$(pandoc "$input" -o "$output" -d ~/.config/pandoc/defaults.yaml 2>&1)
  local exit_code=$?
...
export PATH="/home/rytkmt/.local/bin:$PATH"

function mkPrompt() {
  if [ $# -eq 1 ]; then
    local filepath=".claude/prompts/$1"
